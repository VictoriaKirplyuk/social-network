...
import { UserData } from '../../3-dal/types/registration-api-types';
...
export const registrationComplete = createAsyncThunk('auth/registrationComplete', async (userData: UserData, thunkAPI) => {
  thunkAPI.dispatch(changeStatus({ status: RequestStatus.LOADING }));
  try {
    const continuationCode = sessionStorage.getItem('continuationCode');
