...
import { IUserData } from '../../3-dal/types/registration-api-types';
...
export const registrationComplete = createAsyncThunk('auth/registrationComplete', async (userData: IUserData, thunkAPI) => {
  thunkAPI.dispatch(changeStatus({ status: RequestStatus.LOADING }));
  try {
    const continuationCode = sessionStorage.getItem('continuationCode');

    if (continuationCode) {
      await authAPI.registrationComplete(JSON.parse(continuationCode), userData);
