...
export const messageAPI = {
  ...
  changeMessage(chatId: number, messageId: number, text: string) {
    return instance.put<MessageResponse>(`chat/${chatId}/message/${messageId}`, { text }).then(response => response.data);
  },
  ...
  getMessages(chatId: number, page?: number, size?: number, sort?: string[]) {
    return instance.get<MessagesResponse>(`chat/${chatId}/message?page=${page}&size=${size}&sort=${sort}`).then(response => response.data);
  },
};
